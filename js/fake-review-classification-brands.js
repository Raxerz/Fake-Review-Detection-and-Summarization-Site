...
function fake(category){
	...
	$.ajax({
		...
		url: 'https://product-review-analysis-server.herokuapp.com/getprods?domain='+DOMAIN+'&brandchoice='+BRAND
	}).done(function(data) {
			console.log(data);
			p1=[];
...
fake.prototype.load =function (list1) {
	...
	var fillcards = setInterval(function () {
		...
		var card = ['<div id = "card card-' + i + '" name="'+data[i].name+'" index="'+data[i].index+'"><a href="./fake-review-detection-category.html?index='+(i+1)+'&id='+data[i].id+'">\
...
function init(category="Automotive"){
	...
	localStorage.setItem('nameReviewer',DOMAIN);
